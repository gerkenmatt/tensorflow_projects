...
def eeg_cnn_model_5deep_fn(features, labels, mode):
  ...
  conv5 = tf.layers.conv1d(
    ...
    activation = tf.nn.relu)
  print("\nCONV5 OUTPUT SHAPE: \n", str(conv5.shape))
  max_pool_5 = tf.layers.max_pooling1d(
    inputs=conv5, 
    pool_size=2, 
    strides=2, 
    padding='same')
  print("\nPOOL4 OUTPUT SHAPE: \n", str(max_pool_5.shape))

  # Flatten tensor into a batch of vectors
  # Input Tensor Shape: [batch_size, 160, 192]
  # Output Tensor Shape: [batch_size, 160 * 192]
  pool5_flat = tf.reshape(max_pool_5, [-1, max_pool_5.shape[1]*  512])
  print("\nPOOL_FLAT OUTPUT SHAPE: \n", str(pool5_flat.shape))
